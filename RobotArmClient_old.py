...
last_position_change_time = [0] * 4
idle_timeout = 10.0
idle_threshold = 0.01

start_moving = True

# --- FIX 1: Initialize global variable preventing crash ---
posOffset = 0 
stored_positions = [0,0,0]
serial_Pico = None # Initialize to None
...
# Find ports
# Note: Standard RPi Pico is often VID 0x2E8A, PID 0x0005 (MicroPython)
...
# --- FIX 2: ABORT EARLY IF PICO IS MISSING ---
if pico_port:
    ...
    try:
        ...
        sys.exit(1) # Exit if port exists but cannot be opened
else:
    print("\n❌ CRITICAL ERROR: Pico not found.")
    print("   The Pico is essential. Aborting script.")
    print("\n🔍 Troubleshooting: Here are the devices currently connected:")
    
    # Print all connected devices to help you find the correct VID/PID
    ports = serial.tools.list_ports.comports()
    if not ports:
        print("   (No serial devices detected at all!)")
    for p in ports:
        print(f"   - {p.device}: VID={hex(p.vid) if p.vid else 'None'}:PID={hex(p.pid) if p.pid else 'None'} ({p.description})")
    
    print("\n   👉 Check if your VID/PID (0x239A:0x80F4) matches the list above.")
    print("   Standard Raspberry Pi Pico is usually 0x2E8A:0x0005")
...
# --- FIX 3: DELETE OR COMMENT THIS BLOCK TO PREVENT "DOUBLE OPEN" ERROR ---
...
def read_serial():
    # Safety check if serial_Pico was somehow skipped (though abort logic handles this)
    if serial_Pico is None or not serial_Pico.is_open:
        return
...
def poll_flask():
    global posOffset # Declare global to modify it
    motorPositions = [0.0] * 8
    ...
    while running_event.is_set():
        try:
            ...
            # Invert direction for first two axes
            ...
            if SPM_Waveshare:
                ...
                controller.set_multiple_target_angles({
                    1: UpperRing * SERVO_INVERSIONS[1],
                    2: MiddleRing * SERVO_INVERSIONS[2],
                    3: LowerRing * SERVO_INVERSIONS[3]
                })

        except requests.exceptions.Timeout:
            pass
        except Exception as e:
            print(f"Error in poll_flask: {e}")
...
# nano_port = find_device(0x1A86, 0x7523)
# waveshare_servo_port = find_device(0x1A86, 0x55D3)
# serial_Pico = None  # Initialize serial port for Pico
...
#         print("⚠️ Pico not connected. Serial reader thread idling.")
#         return # Or loop with sleep if you expect it to plug in later
#     while running_event.is_set():
#         try:
#             line = serial_Pico.readline().decode("utf-8").strip()
#             if line:
